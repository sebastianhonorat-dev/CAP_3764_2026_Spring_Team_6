...
def collect_national(per_page: int = 100) -> pd.DataFrame:
    fields = ",".join([
        "id",
        "school.name",
        "school.state",          # ← add this so you can use it in stage 1
        ...
        "latest.student.demographics.age_entry",
        "latest.school.title_iv.eligibility_type",
        "latest.admissions.sat_scores.average.overall",        # student quality
        "latest.admissions.act_scores.midpoint.cumulative",    # student quality alternative
        "latest.student.grad_students",                        # grad school presence
    ...
    per_page_actual = int(data["metadata"]["per_page"])
    total_pages = math.ceil(total / per_page_actual)
    print(f"Total schools to fetch: {total} across {total_pages} pages")

    META = [
        "id",
        "school.name",
        "school.state",          # ← add here too
        "location.lat",
        "location.lon",
        "latest.school.locale",
        "latest.school.carnegie_size_setting",
    ...
    print(f"Total pages fetched: {total_pages}")
    print(f"Total rows/programs: {len(df)}")
    print(f"Unique schools: {df['id'].nunique()}")
    return df
